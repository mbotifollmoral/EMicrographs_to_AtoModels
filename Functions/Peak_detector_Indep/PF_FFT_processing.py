...
def experimental_fft(sample, window_size = 2048, zone = [0,0]):
    ...
    if window_size==x_pixels_original:
        ...
        #init_y=np.random.choice(np.arange(0,total_image.shape[0]-window_size,1))
        #init_x=np.random.choice(np.arange(0,total_image.shape[1]-window_size,1))
        init_y = zone[1]
        init_x = zone[0]
...
def standarize_fft(signal, fft, opt_scale = 0.05):
    # opt_scale = 0.025
    ...
    if ratio == 1:
        standarize_fft = fft
        lim_l = 0
        lim_r = int(fft.shape[0])
        
    elif ratio < 1:
        # pixel size of the input image is bigger than the optimal scale
    
        exp = int(np.log2(ratio))
        real_size = len(fft)
        estandarize_size = int(real_size/(2**exp))
        
        standarize_fft = np.zeros((estandarize_size, estandarize_size))
        lim_l = int(np.floor(estandarize_size/2 - fft.shape[0]/2))
        lim_r = int(np.floor(estandarize_size/2 + fft.shape[0]/2))
        
        standarize_fft[lim_l : lim_r, lim_l : lim_r] = fft
        
    else:
        # pixel size of the input image is smaller than the optimal scale
    
        exp = int(np.log2(ratio))
        real_size = len(fft)
        estandarize_size = int(real_size/(2**exp))
        
        lim_l = int((real_size - estandarize_size)//2)
        lim_r = int((real_size + estandarize_size)//2)
        
        standarize_fft = fft[lim_l : lim_r, lim_l : lim_r]
    
    return  standarize_fft, lim_l, lim_r



def standarize_fft_modFOV(
        signal, fft, opt_scale = 0.05, opt_fov = 15):
    # opt_scale = 0.025
    scale = signal.axes_manager['x'].scale
    ...
    if ratio == 1:
        ...
        # we get hte image cut or adding the patch
        exp = int(np.log2(ratio))
        real_size = len(fft)
        estandarize_size = int(real_size/(2**exp))
    ...
    # modify the FOV  directly from the standarized image  
      
    # fov_final = fov_factor * fov_init
      
    fov_original = scale*fft.shape[0]
    
    
    
    fov_factor = opt_fov/fov_original
    
    totpix_factor = (2**int(np.log2(fov_factor)))/(2**int(np.log2(ratio)))
    
    newsize = (int(fft.shape[0]*totpix_factor), int(fft.shape[0]*totpix_factor))
    
    print('fov_original')
    print(fov_original)
    print('fov_factor')
    print(fov_factor)
    print((2**int(np.log2(fov_factor))))
    print((2**int(np.log2(ratio))))
    ...
    print('newsize')
    print(newsize)
    new_standarize_fft = cv2.resize(
        standarize_fft, dsize=newsize, interpolation=cv2.INTER_LINEAR)
    
    # dsdasdasd s new_standarize_fft = cv2.resize(
    #     standarize_fft, dsize=newsize, interpolation=cv2.INTER_NEAREST)
    ...
    if ratio == 1:
        ...
        # rescale the coordinate or limits given the new pixel size
        new_lim_l = np.round((lim_l/standarize_fft.shape[0])*(newsize[0]-1))
        new_lim_r = int(new_lim_l + (lim_r - lim_l)*totpix_factor)
    ...
    return new_standarize_fft, new_lim_l, new_lim_r
