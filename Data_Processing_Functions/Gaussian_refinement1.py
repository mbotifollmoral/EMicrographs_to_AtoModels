# -*- coding: utf-8 -*-
"""
Created on Fri Nov 13 19:34:12 2020
...
'''
Strategies for refining the K-means clustering
1) By defining the centroids of the method by the centers of the gaussians and its fitting in the histogram, and applying only a possible small
shift of the centroid with respect the initial position, given to only these peaks that very clearly fit the gaussian and are narrow enough
2) By directly fitting a gaussian to these peaks that very clearly fit the gaussian and are narrow enough, and set these pixels to that cluster, 
and the remaining ones must be clustered normally by mean shift or k means or other clustering routines
'''
...
#ds_image_st=Custom_Filter(ds_image_st, nm_size=50)
...
def BGMM(image_,number_clusters):
    ...
    image_reshaped=np.reshape(image_, (image_.shape[0]*image_.shape[1],1))
    gmm = sklearn.mixture.BayesianGaussianMixture(n_components=number_clusters)
    gmm.fit(image_reshaped)
...
#Only Gaussian routine

number_clusters_final=3
labels, means, covs=BGMM(ds_image_st,number_clusters=number_clusters_final)
print(means)
print(covs)

number_final_regions=6
ordered_clusters=Multiple_Consecutive_Clustered_regions(labels, criteria='Num_Clusters', tolerance=0, n_clusters=number_final_regions)


final_image=np.zeros(np.shape(labels))
value_image=1
...
vertexs_2=conts_vertx_per_region['2_vertexs']
vertexs_1=conts_vertx_per_region['1_vertexs']

xs=[]
ys=[]
for vertex in vertexs_1:
    y=vertex[0]
    ys.append(y)
    x=vertex[1]
    xs.append(x)

for vertex in vertexs_2:
    y=vertex[0]
    ys.append(y)
    x=vertex[1]
    xs.append(x)
...
image_to_refine=np.copy(ds_image_st)
...
plt.imshow(image_to_refine, cmap=plt.cm.gray, vmin=image_to_refine.min(), vmax=image_to_refine.max())
plt.show()   

plt.hist(image_to_refine.ravel(),256,[np.min(np.array([image_to_refine])),np.max(np.array([image_to_refine]))])
plt.show()
...
#Gaussian refined process



image_to_refine=Gaussian_pre_clustering(ds_image_st,number_of_gaussians=3,variance_threshold=0.001)
...
k_means_clusters=3
values, labels, cost = best_km(image_to_refine, n_clusters =k_means_clusters)
labels_ms_reshaped = np.choose(labels, values)
labels_ms_reshaped.shape = ds_image_st.shape


plt.imshow(labels_ms_reshaped,cmap=plt.cm.gray , vmin=0, vmax=3)
plt.show()
