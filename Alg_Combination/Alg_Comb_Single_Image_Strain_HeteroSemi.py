...
segm_setting = 'crop'  # Setting to define if use SAM segm, 'SAM', or contour, else
...
dataset_system_path_name = r'E:\Arxius varis\PhD\4rth_year\Global_Results_per_Device\InSb_InP_TransvNW_3_heterocode\Micrographs\\'
...
GPA_save_directory = Results_dir + 'GPA_output_' + str(label_of_GPA_ref) + '\\'
...
# store the filepaths to the model cells built upon each label being a reference
# and labels equally oriented found at that given reference region
all_atom_models_filepaths = [atom_models_filepath]
all_labels_equally_oriented_as_ref = [labels_equally_oriented_as_ref]
all_labels_GPAed = [label_of_GPA_ref]

#%% 

'''
Addition to the heterostructure refinement
'''

if len(paths_to_virt_ucells) == 0:
    raise ValueError('Not enough crystals to find an heterostructure')
    
else:
    # extract the labels from which we get spots inside the mask range found
        
    labels_virt = []
    
    for virt_path in paths_to_virt_ucells:
        
        int_path = virt_path[:virt_path.find('_v.cif')]
        
        label_virt =  int(int_path[int_path.rfind('_')+1:])
        
        labels_virt.append(label_virt)
        


# Perform the GPA calculation taking as references the other values of labels extracted

new_paths_to_v_unitcells = []

for label_virt_GPA, scored_spot_pair_found_virt, scaled_cordspot_virt in zip(
        labels_virt, scored_spot_pairs_found, scaled_cords_spots):
    ...
    # This is to automate the definition of the region where the substrate is found
    # to take the reference from it
    label_of_GPA_ref = label_virt_GPA
    label_of_substrate = GPA_sp.Define_Region_as_GPA_Reference(
        analysed_image_only, images_segmented[0])
    ...
    # Retrieve the best spot pair to be considered the GPA g vectors
    # if both coordinates are the same, then only one spot is computed in the
    # region inside the mask, therefore find both
    if scaled_cordspot_virt[0][0] == scaled_cordspot_virt[1][0] and scaled_cordspot_virt[0][1] == scaled_cordspot_virt[1][1]:
    
        best_GPA_ref_spot_pair = GPA_sp.Get_GPA_best_g_vects_pair(
            analysed_image_only, label_of_GPA_ref, images_segmented[0])
    
    else:
        # if they are not the same, so two points are found then use the 
        #spot pair already as the best to be used for GPA
        best_GPA_ref_spot_pair = scored_spot_pair_found_virt
    ...
    '''
    Translation of GPA information, displacement fields, into the atomistic model
    for each label in the virtual cells
    '''
    ...
    # Append the unitcells into the new list
    new_paths_to_v_unitcells.append(path_to_v_unitcell)
    ...
    # store the paths to the models built 
    all_atom_models_filepaths.append(atom_models_filepath)
    all_labels_equally_oriented_as_ref.append(labels_equally_oriented_as_ref)
    all_labels_GPAed.append(label_of_GPA_ref)
...
# keep track of the final file which is created after straining and purging
paths_global_strained_purgeds = []


for atom_model_cell_path in all_atom_models_filepaths:
    
    # make the transfer idenepdently for every folder and its label
    ...
    path_global_strained_purged = GPA_AtoMod.Distort_AtoModel_Region(
        atom_model_cell_path, Disp_x, Disp_y, Box_strain_pixels, 
    ...
    paths_global_strained_purgeds.append(path_global_strained_purged)



# then after all of this is done
# make all the models in ever directory have an extra string with the lable
# so add to the name a _label.xyz
# and then copy them all in a single folder and do the final reginement out of this


# !!! need to add the final label _label.xyz in the region_cut_strained_purged.xyz
# to be able to cut it in the segmented checker in the 
# Refine_StrainedRegion_MultiAtomBlock_Segmentation



# path to all models so to folder, from which to extract the lable
# all_atom_models_filepaths


# path to the strained purged model, to add the label when renaming it
# paths_global_strained_purgeds

renamed_paths_global_strained_purgeds =[]

for label_GPAed, path_global_strained_purg in zip(
        all_labels_GPAed, paths_global_strained_purgeds):
    
    
    renamed_path_global_strained_purged = path_global_strained_purg[:path_global_strained_purg.find('purged')+6] + '_' + str(label_GPAed) + '.xyz'
    
    new_filename = 'region_cut_strained_purged_' +  str(label_GPAed) + '.xyz'
    
    # rename the file in path_global_strained_purg
    # copy and paste and renamedoes not work, need to rename it
    # to ..._label.xyz
    os.rename(
        path_global_strained_purg, renamed_path_global_strained_purged)
    
    renamed_paths_global_strained_purgeds.append(renamed_path_global_strained_purged)
...
# store the models cut given the segmentation and the labels
paths_to_final_segment_refined_models = []
...
for atom_model_cell_path in all_atom_models_filepaths:
    ...
    atomregmodel_path_final = GPA_AtoMod.Refine_StrainedRegion_MultiAtomBlock_Segmentation(
        atom_model_cell_path, conts_vertx_per_region)

    paths_to_final_segment_refined_models.append(atomregmodel_path_final)
...
'''
Combine the models into one single file
'''


def Combine_xyz_supercells_finals(
        paths_to_final_segment_refined_models, new_folder_path_final):
    '''
    ...
    for file_name in paths_to_final_segment_refined_models:
    ...
    global_supercell_name = new_folder_path_final +'\\' + 'Final_merged_atomic_model.xyz'
...
new_folder_path_final = all_atom_models_filepaths[:all_atom_models_filepaths.find('_strained_')+10] + '_merged\\'

Combine_xyz_supercells_finals(
    paths_to_final_segment_refined_models, new_folder_path_final)
